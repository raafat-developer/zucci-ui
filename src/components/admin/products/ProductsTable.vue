<template>
  <div class="zp-table-panel">
    <div v-if="selectedIds.length" class="zp-bulk-bar">
      ...
      <select
        class="zp-filter-select"
        :value="bulkAction"
        @change="$emit('update:bulk-action', $event.target.value)"
      >
        ...
        >
          ...
          <span class="zp-col-product">
            ...
            <div class="zp-product-info">
              ...
              <span v-if="product.isVariable" class="zp-variant-count"
                >{{ product.variantCount }} variants</span
              >
              <span
                v-if="product.returnPolicy === 'non_returnable'"
          ...
          <span class="zp-col-sku"
            ><span class="zp-sku">{{ product.sku }}</span></span
          >
          <span class="zp-col-markets">
            <span
              v-for="market in product.markets || []"
              :key="market"
              class="zp-market-pill"
              >{{ markets.find((m) => m.id === market)?.code }}</span
            >
          </span>
          <span class="zp-col-inventory">
            <span :class="{ 'zp-stock-zero': product.inventoryTotal === 0 }"
              >{{ product.inventoryTotal }} pcs</span
            >
          </span>
          <span class="zp-col-status">
            <span
              ...
              :style="{
                background: statusMeta(product.status).bg,
                color: statusMeta(product.status).color,
              }"
              >{{ statusMeta(product.status).label }}</span
            >
          </span>
          <span class="zp-col-approval">
            <span
              style="
                ...
                font-weight: 600;
              "
              :style="{ color: approvalMeta(product.approvalStatus).color }"
              >{{ approvalMeta(product.approvalStatus).label }}</span
            >
          </span>
          <span class="zp-col-sync">
            <span
              style="font-size: 10.5px; font-family: var(--zg-mono)"
              :style="{ color: syncMeta(product.syncSource).color }"
              >{{ syncMeta(product.syncSource).label }}</span
            >
            <span v-if="product.syncStatus === 'synced'" class="zp-sync-dot" />
          </span>
          <span class="zp-col-actions">
            <button
              type="button"
            ...
            <button
              @click.stop="$emit('quick-approve', product)"
              v-if="product.approvalStatus === 'pending'"
              class="zen-act-btn-sm"
              title="Quick approve"
              style="color: var(--zg-good)"
            >
              <svg
                viewBox="0 0 14 14"
                width="11"
                ...
                stroke-linecap="round"
              >
                <path d="M2 7L6 11L12 3"></path>
              </svg>
            </button>
          </span>
        </div>

        <div v-if="error" class="zp-empty" style="color: var(--zg-danger)">
          {{ error }}
        </div>
        <div v-else-if="!products.length" class="zp-empty">
          {{
