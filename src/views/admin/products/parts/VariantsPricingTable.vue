...
const displayMkts = computed(() =>
  props.activeMkts.length ? props.activeMkts : ZCP_ALL_MARKETS.slice(0, 3),
);
const mktCur = computed(
  () =>
    ZCP_ALL_MARKETS.find((m) => m.code === mktTab.value)?.currency ||
    mktTab.value,
);
...
function generate() {
  ...
  const rawSizes = attrs.size?.length ? attrs.size : ["One Size"];
  ...
  colors.forEach((col) =>
    sizes.forEach((sz) => {
      gen.push({
        ...
        style: (attrs.style || [])[0] || "",
        sourceCurrency: props.product.currency || "AED",
        sourcePrice: props.product.price || 0,
        commissionPct: 12,
        netVendor: Math.round((props.product.price || 0) * 0.88),
        marketPrices: Object.fromEntries(
          ZCP_ALL_MARKETS.map((m) => [m.code, 0]),
        ),
        recommendedLow: null,
        ...
        sku:
          catCode +
          "-" +
          (props.product.brandCode || "BRD") +
          "-SC25-" +
          col.slice(0, 3).toUpperCase() +
          "-" +
          String(sz).toUpperCase(),
      });
    }),
  );
...
const isWarn = (v) => {
  ...
  const lo = recLow(v);
  const hi = recHi(v);
  return lo && hi && mp > 0 && (mp < lo || mp > hi);
};

function updatePrice(v, val) {
  const num = parseFloat(val) || 0;
  const updatedVariants = props.product.variants.map((item) => {
    if (item.id === v.id) {
      const marketPrices = { ...(item.marketPrices || {}) };
      marketPrices[mktTab.value] = num;
      const commissionPct = item.commissionPct || 15;
      const netVendor = Math.round(num * (1 - commissionPct / 100));
    ...
    <div v-if="!hasVariants" class="zp-variants-empty">
      <div>
        <div
          ...
          "
        >
          No variants configured. Add attribute values above, then generate.
        </div>
        <div style="font-size: 11px; color: var(--zg-text-dim)">
          Variants are generated from every Color × Size combination.
        </div>
      </div>
      <button
        type="button"
        class="zen-btn-primary"
        style="height: 30px; font-size: 11.5px; flex-shrink: 0"
        @click="generate"
      >
      ...
      <table class="zp-var-table">
        <thead>
          <tr>
            ...
            <th style="font-size: 9px; color: var(--zg-text-dim)">
              Rec. Range
            </th>
            <th>Stock</th>
          </tr>
        ...
        <tbody>
          <tr v-for="v in variants" :key="v.id" class="zp-var-row">
            ...
            <td>
              <span class="zp-price-val"
                >{{ v.sourceCurrency }}
                {{ (v.sourcePrice || 0).toLocaleString() }}</span
              >
            </td>
            <td>
              <span style="font-size: 10px; color: var(--zg-text-dim)"
                >{{ v.commissionPct }}%
              </span>
              <span class="zp-price-neg"
                >−{{
                  Math.round(((v.sourcePrice || 0) * v.commissionPct) / 100)
                }}</span
              >
            </td>
            <td>
              <span class="zp-price-val" style="color: var(--zg-good)"
                >{{ v.sourceCurrency }} {{ v.netVendor }}</span
              >
            </td>
            <td>
              <div style="display: flex; align-items: center; gap: 4px">
                <input
                  type="number"
                  class="zp-price-input zp-field-h"
                  :value="priceFor(v)"
                  @input="updatePrice(v, $event.target.value)"
                  :style="{
                    borderColor: isWarn(v) ? 'var(--zg-warn)' : undefined,
                  }"
                />
                <span
                  v-if="isWarn(v)"
                  title="Outside recommended range"
                  style="color: var(--zg-warn); font-size: 11px"
                  >⚠</span
                >
              </div>
            </td>
            <td>
              <span style="font-size: 10px; color: var(--zg-text-dim)">{{
                recLow(v) && recHi(v) ? `${recLow(v)}–${recHi(v)}` : "—"
              }}</span>
            </td>
