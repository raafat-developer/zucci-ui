...
const VENDOR_TIERS = reactive([
  { id:'platinum', label:'Platinum', color:'oklch(0.85 0.06 280)', gmvMin:500000, commission:8,  benefits:['Dedicated AM','Priority listing','0% storage fee for 60d','Monthly payout'] },
  { id:'gold',     label:'Gold',     color:'oklch(0.82 0.15 75)',  gmvMin:100000, commission:10, benefits:['Bi-weekly payout','Featured placement','Reduced return rate fee'] },
...
<template>
  <div class="zset-content">
    <div class="zset-content-head">
      <div>
        <div class="zset-content-title">Tiers Management</div>
        <div class="zset-content-sub">Configure vendor and customer tier thresholds, commissions, and benefits</div>
      </div>
      <button class="btn-primary" @click="">+ Add Tier</button>
    </div>

    <!-- Group switch -->
    <div style="display:flex;gap:4px;margin-bottom:16px;">
      <button v-for="[v,l] in [['vendor','Vendor Tiers'],['customer','Customer Tiers']]" :key="v"
        class="tier-tab" :class="{ 'is-active': activeGroup === v }"
        @click="activeGroup = v">{{ l }}</button>
    </div>

    <!-- Vendor Tiers -->
    <template v-if="activeGroup === 'vendor'">
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:10px;">
        <div v-for="tier in VENDOR_TIERS" :key="tier.id"
          ...
          <div style="display:flex;align-items:center;gap:10px;">
            <div style="width:12px;height:12px;border-radius:50%;" :style="{ background: tier.color }" />
            <span style="font-size:14px;font-weight:800;color:var(--zg-text);">{{ tier.label }}</span>
          </div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
            ...
            <div>
              <div style="font-size:9.5px;text-transform:uppercase;letter-spacing:.07em;color:var(--zg-text-dim);font-weight:600;margin-bottom:3px;">Commission</div>
              <div style="font-family:var(--zg-mono);font-size:14px;font-weight:700;" :style="{ color: tier.color }">{{ tier.commission }}%</div>
            </div>
          </div>
          <div>
          ...
          <button class="act-btn" @click="">Edit Tier</button>
        </div>
      </div>
    </template>
    ...
    <template v-else>
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:10px;">
        <div v-for="tier in CUSTOMER_TIERS" :key="tier.id"
          style="background:var(--zg-panel);border:1px solid var(--zg-line);border-radius:var(--zg-radius-md);padding:16px;display:flex;flex-direction:column;gap:10px;">
          <div style="display:flex;align-items:center;gap:10px;">
            <div style="width:12px;height:12px;border-radius:50%;" :style="{ background: tier.color }" />
            <span style="font-size:14px;font-weight:800;color:var(--zg-text);">{{ tier.label }}</span>
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
            ...
            <div>
              <div style="font-size:9.5px;text-transform:uppercase;letter-spacing:.07em;color:var(--zg-text-dim);font-weight:600;margin-bottom:3px;">Discount</div>
              <div style="font-family:var(--zg-mono);font-size:14px;font-weight:700;" :style="{ color: tier.discount > 0 ? tier.color : 'var(--zg-text-dim)' }">{{ tier.discount > 0 ? tier.discount+'%' : '—' }}</div>
            </div>
          </div>
          <div>
            <div style="font-size:9.5px;text-transform:uppercase;letter-spacing:.07em;color:var(--zg-text-dim);font-weight:600;margin-bottom:6px;">Benefits</div>
...
<style scoped>
.tier-tab { height:30px;padding:0 14px;border:1px solid var(--zg-line);border-radius:var(--zg-radius-md);background:transparent;cursor:pointer;font-size:12px;color:var(--zg-text-dim); }
.tier-tab.is-active { background:var(--zg-panel-hi);color:var(--zg-text);border-color:var(--zg-accent); }
