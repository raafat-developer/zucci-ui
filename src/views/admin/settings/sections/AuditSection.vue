...
const maxPage = computed(() => Math.ceil(filtered.value.length / PAGE_SIZE) - 1);

function exportCSV() {
  const rows = [['ID','User','Action','Module','Time','IP'], ...AUDIT_LOG.map(e => [e.id, e.user, e.action, e.module, e.time, e.ip])];
  const csv = rows.map(r => r.join(',')).join('\n');
  const b = new Blob([csv], { type:'text/csv' }); const a = document.createElement('a'); a.href = URL.createObjectURL(b); a.download = 'audit-log.csv'; a.click();
}
</script>
...
<template>
  <div class="zset-content">
    <div class="zset-content-head">
      <div><div class="zset-content-title">Audit Log</div><div class="zset-content-sub">All admin actions with user, module, and timestamp</div></div>
      <button class="btn-ghost" @click="exportCSV">Export CSV</button>
    </div>
    <div style="position:relative;margin-bottom:12px;max-width:300px;">
      ...
      <input v-model="search" placeholder="Search by user, action, module…" style="height:28px;padding-left:24px;padding-right:8px;width:100%;background:var(--zg-panel);border:1px solid var(--zg-line);border-radius:var(--zg-radius-md);font-size:12px;color:var(--zg-text);outline:none;box-sizing:border-box;" @input="page = 0" />
    </div>
    <div style="border:1px solid var(--zg-line);border-radius:var(--zg-radius-md);overflow:hidden;">
      <div style="display:grid;grid-template-columns:80px 1fr 1fr 80px 80px;padding:0 14px;height:30px;background:var(--zg-panel-hi);border-bottom:1px solid var(--zg-line);font-size:9.5px;text-transform:uppercase;letter-spacing:.07em;color:var(--zg-text-dim);font-weight:600;align-items:center;gap:10px;">
        <span>ID</span><span>Action</span><span>Module</span><span>Time</span><span>IP</span>
      </div>
      <div v-for="e in paged" :key="e.id" style="display:grid;grid-template-columns:80px 1fr 1fr 80px 80px;padding:9px 14px;background:var(--zg-panel);border-bottom:1px solid var(--zg-line);align-items:center;font-size:12px;gap:10px;">
        ...
        <span style="font-size:11px;color:var(--zg-text-dim);">{{ e.module }}</span>
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;margin-top:8px;font-size:11.5px;color:var(--zg-text-dim);">
      ...
      <div style="display:flex;gap:6px;">
        <button class="btn-ghost" style="height:26px;font-size:11px;" :disabled="page === 0" :style="{ opacity: page===0?0.4:1 }" @click="page--">← Prev</button>
        <span style="padding:0 8px;display:flex;align-items:center;">{{ page + 1 }} / {{ maxPage + 1 }}</span>
        <button class="btn-ghost" style="height:26px;font-size:11px;" :disabled="page >= maxPage" :style="{ opacity: page>=maxPage?0.4:1 }" @click="page++">Next →</button>
      </div>
    </div>
  </div>
</template>
