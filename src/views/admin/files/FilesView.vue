...
import { FOLDERS } from "@/data/filesData";
import { getFiles, uploadFile, deleteFile as deleteFileApi } from "@/api/files";
...
async function fetchFiles() {
  ...
  try {
    const params = {
      ...
      type: typeF.value !== "all" ? typeF.value : undefined,
      folder: folder.value !== "all" ? folder.value : undefined,
    };
    const response = await getFiles(params);
    const responseData = response.data?.data || response.data || [];
    const responseMeta = response.data?.meta || {};

    files.value = responseData.map((f) => ({
      id: f.id || f.fileId,
      ...
      size: f.sizeBytes
        ...
        : "—",
      dims: "—",
      folder: f.fileCategoryId || "images",
      uploaded: f.createdAt ? new Date(f.createdAt).toLocaleDateString() : "—",
      uploader: f.uploadedBy || "—",
      url: f.url,
      tags: [],
    }));
    pagination.value.page = responseMeta.current_page || 1;
    pagination.value.pageSize = responseMeta.per_page || 25;
    pagination.value.total = responseMeta.total || 0;
    pagination.value.totalPages = responseMeta.last_page || 1;
  } catch (error) {
  ...
  FOLDERS.map((f) => ({
    ...
    count:
      f.id === "all"
        ...
        : files.value.filter((x) => x.folder === f.id).length,
  })),
);
  ...
  return files.value.filter(
    (f) =>
      ...
      (!sq ||
        ...
        f.tags.some((t) => t.includes(sq)) ||
        f.uploader.toLowerCase().includes(sq)),
  );
});
...
async function deleteSelected() {
  ...
  try {
    await Promise.all(ids.map((id) => deleteFileApi(id)));
...
async function onGlobalDrop(e) {
  ...
  for (let i = 0; i < droppedFiles.length; i++) {
    ...
    try {
      ...
      uploading.value = uploading.value.map((x) =>
        x.id === upload.id ? { ...x, progress: 100, done: true } : x,
      );
      setTimeout(() => {
        uploading.value = uploading.value.filter((x) => x.id !== upload.id);
      }, 2000);
      await fetchFiles();
    } catch (error) {
      console.error("Failed to upload file:", error);
...
async function handleDeleteFile(id, file) {
...
<template>
  <div
    ...
    @dragleave="
      (e) => {
        if (!$el.contains(e.relatedTarget)) globalDrag = false;
      }
    "
    @drop="onGlobalDrop"
  >
    ...
    <div v-if="uploading.length" class="zfl-progress-stack">
      <div v-for="u in uploading" :key="u.id" class="zfl-progress-card">
        <div class="zfl-progress-row">
          ...
          <span class="zfl-progress-pct" :class="{ 'is-done': u.done }">{{
            u.done ? "Done" : u.progress + "%"
          }}</span>
        </div>
        <div class="zfl-progress-bar">
          <div
            class="zfl-progress-fill"
            :class="{ 'is-done': u.done }"
    ...
    <div class="zwh-header">
      <div class="zwh-header-left">
        ...
        <div class="zwh-subtitle">
          {{ pagination.total }} files · {{ totalSize }} MB in current view
        </div>
      </div>
      <div class="zfl-header-actions">
        <button
          v-if="selected.size > 0"
          class="zfl-btn-delete-sel"
        ...
        <button
          v-if="selected.size > 0"
          class="zwh-btn-ghost zfl-btn-desel"
          @click="clearSelect"
        >
          Deselect
        </button>
        <button
          class="zfl-view-toggle"
        ...
        >
          ...
          <svg v-else viewBox="0 0 12 12" width="12" fill="currentColor">
      ...
      <aside class="zfl-sidebar">
        ...
        <nav class="zfl-folder-nav">
          ...
          >
            ...
            >
              <path
                d="M1 4c0-1 .6-1.5 1.5-1.5h4l1.5 1.5H14c.8 0 1.5.6 1.5 1.5v7c0 .8-.7 1.5-1.5 1.5H2.5C1.7 14 1 13.3 1 12.5V4z"
              />
            </svg>
      ...
      <div class="zfl-main">
        ...
        <div class="zfl-toolbar">
          <div class="zfl-search-wrap">
            ...
            <input
              v-model="search"
              placeholder="Search by name, tag, uploader…"
              class="zfl-search"
            />
          </div>
          <button
            v-for="t in ['all', 'image', 'pdf', 'doc', 'video']"
            :key="t"
        ...
        <div class="zfl-content">
          <div
            class="zfl-scroll"
            :class="viewMode === 'grid' ? 'is-grid-pad' : ''"
          >
            <div v-if="loading" class="zfl-empty">
              <span>Loading files…</span>
            </div>
            <div v-else-if="filtered.length === 0" class="zfl-empty">
              <svg
                viewBox="0 0 24 24"
                width="40"
                fill="none"
            ...
            <template v-else>
              <div class="zfl-list-head">
                ...
                ><span>Size</span><span>Dimensions</span><span>Uploaded</span>
              </div>
              <FileRow
          ...
          <!-- Pagination -->
          <div v-if="pagination.totalPages > 1" class="zfl-pagination">
            <button
              class="zfl-page-btn"
...
.zfl-global-drop {
  ...
  border: 3px dashed var(--zg-accent);
...
.zfl-btn-delete-sel {
  height: 30px;
  padding: 0 12px;
  border: 1px solid var(--zg-danger);
  border-radius: var(--zg-radius-md);
