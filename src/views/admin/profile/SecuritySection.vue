  ...
  <div>
    ...
    <div class="up-card" style="margin-bottom: 12px">
      <div
        ...
        "
        :style="{ marginBottom: pwStep ? '14px' : 0 }"
      >
        <div>
          <div style="font-size: 13px; font-weight: 600; color: var(--zg-text)">
            Password
          </div>
          <div
        ...
        <button class="up-btn-ghost" @click="pwStep = !pwStep">
          {{ pwStep ? "Cancel" : "Change Password" }}
        </button>
      </div>
      <div
        v-if="pwStep"
    ...
    <div class="up-card" style="margin-bottom: 12px">
      ...
      >
        <div>
          ...
          <div
            style="font-size: 11px; color: var(--zg-text-dim); margin-top: 1px"
          >
            Require TOTP code on every login
          </div>
        </div>
        <button
          class="up-toggle"
      ...
      <div
        ...
        "
      >
        2FA is active — using authenticator app (TOTP)
      </div>
      <div
        v-else
        style="
      ...
      >
        <div>
          ...
          <div
            style="font-size: 11px; color: var(--zg-text-dim); margin-top: 1px"
          >
            Biometric login — no password needed
          </div>
        </div>
      ...
      >
        <div>
          ...
          <div
            ...
            "
          >
            Created {{ pk.created }} · Last used {{ pk.last }}
          </div>
        </div>
        <button
          class="up-act"
          style="color: var(--zg-danger)"
          @click="passkeys = passkeys.filter((x) => x.id !== pk.id)"
        >
          Remove
        </button>
      </div>
