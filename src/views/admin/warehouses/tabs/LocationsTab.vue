...
const totalBins = computed(() =>
  zones.value.flatMap(z => z.aisles || []).flatMap(a => a.bins || []).length
);
const usedBins = computed(() =>
  zones.value.flatMap(z => z.aisles || []).flatMap(a => a.bins || []).filter(b => (b.current || 0) > 0).length
);
...
function binColor(b) {
  ...
  return pct >= 85 ? 'var(--zg-danger)' : pct >= 60 ? 'var(--zg-warn)' : 'var(--zg-good)';
}
</script>
  ...
  <div class="zwh-tab-body">
    ...
    <div v-if="isVirtual" class="zwh-empty">
      ...
      <div class="zwh-empty-sub">
        {{ wh.types.includes('virtual') ? 'Virtual warehouses do not have physical bin locations.' : 'Cross-dock facilities do not use bin-level storage.' }}
      </div>
    </div>
    ...
    <template v-else>
      ...
      <div style="display:flex;gap:16px;align-items:center;flex-wrap:wrap;margin-bottom:10px;">
        <span style="font-size:12px;color:var(--zg-text-dim);">
          {{ zones.length }} zones · {{ zones.flatMap(z=>z.aisles||[]).length }} aisles · {{ totalBins }} bins
        </span>
        <span style="font-size:12px;color:var(--zg-good);">
          {{ usedBins }} occupied ({{ totalBins ? Math.round((usedBins/totalBins)*100) : 0 }}%)
        </span>
        ...
        <div v-for="zone in zones" :key="zone.id" class="zwh-zone-block">
          ...
          <div class="zwh-zone-row" @click="toggleZone(zone.id)">
            ...
            <div class="zwh-zone-util-bar">
              <div class="zwh-zone-util-fill zwh-util-fill"
                :class="`is-${utilCls(utilPct((zone.aisles||[]).flatMap(a=>a.bins||[]).reduce((s,b)=>s+(b.current||0),0), (zone.aisles||[]).flatMap(a=>a.bins||[]).reduce((s,b)=>s+(b.max||0),0)))}`"
                :style="{ width: utilPct((zone.aisles||[]).flatMap(a=>a.bins||[]).reduce((s,b)=>s+(b.current||0),0),(zone.aisles||[]).flatMap(a=>a.bins||[]).reduce((s,b)=>s+(b.max||0),0)) + '%', height:'100%', borderRadius:'2px' }" />
            </div>
            <svg class="zwh-zone-chevron" :class="{ open: openZones[zone.id] }" viewBox="0 0 8 12" width="7" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"><path d="M2 2l4 4-4 4"/></svg>
          </div>

          <!-- Aisles -->
          <div v-if="openZones[zone.id]" class="zwh-zone-aisles">
            <div v-for="aisle in (zone.aisles||[])" :key="aisle.id" class="zwh-aisle-block">
              <div class="zwh-aisle-row" @click="toggleAisle(`${zone.id}-${aisle.id}`)">
                <span class="zwh-aisle-id">{{ aisle.id }}</span>
                <span class="zwh-aisle-name">{{ aisle.name || ('Aisle ' + aisle.id) }}</span>
                <span class="zwh-aisle-stat">{{ (aisle.bins||[]).length }} bins</span>
                <svg class="zwh-zone-chevron" :class="{ open: openAisles[`${zone.id}-${aisle.id}`] }" viewBox="0 0 8 12" width="7" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"><path d="M2 2l4 4-4 4"/></svg>
              </div>
              <!-- Bins grid -->
              <div v-if="openAisles[`${zone.id}-${aisle.id}`]" class="zwh-bins-grid">
                <div v-for="b in (aisle.bins||[])" :key="b.id" class="zwh-bin-chip"
                  :title="`${b.id}: ${b.current||0}/${b.max||0}`"
                  :style="{ borderColor: binColor(b), color: binColor(b) }">
                  <span style="font-size:9.5px;font-family:var(--zg-mono);">{{ b.id }}</span>
                  <span style="font-size:9px;opacity:0.7;">{{ b.current||0 }}/{{ b.max||0 }}</span>
                </div>
              </div>
            </div>
          </div>
