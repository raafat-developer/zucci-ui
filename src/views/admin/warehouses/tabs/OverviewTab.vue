...
import { ZCW_ALERTS as SEED_ALERTS, ZCW_ACTIVITY } from "@/data/warehouseData";
...
const alerts = computed(() =>
  SEED_ALERTS.filter((a) => a.warehouse_id === props.wh.id),
);
...
const STATS = computed(() => [
  {
    lbl: "Total SKUs",
    val: props.wh.stats.total_skus.toLocaleString(),
    cls: "accent",
  },
  { lbl: "Active Vendors", val: props.wh.stats.active_vendors, cls: "" },
  {
    lbl: "Pending Inbound",
    val: props.wh.stats.pending_inbound,
    cls: props.wh.stats.pending_inbound > 8 ? "warn" : "",
  },
  { lbl: "Pending Outbound", val: props.wh.stats.pending_outbound, cls: "" },
  {
    lbl: "Returns Queue",
    val: props.wh.stats.pending_returns,
    cls: props.wh.stats.pending_returns > 5 ? "warn" : "",
  },
...
const gauges = computed(() => {
  ...
  return [
    c.m2 > 0
      ? { label: "Floor Space", used: c.used_m2, total: c.m2, unit: "m²" }
      : null,
    c.pallets > 0
      ? {
...
<template>
  <div class="zwh-tab-body">
    ...
    <div class="zwh-ov-grid">
      ...
      <div class="zwh-card-panel">
        ...
        <div class="zwh-panel-body">
            ...
            <div v-for="g in gauges" :key="g.label" class="zwh-gauge-row">
              ...
              >
                ...
                <span
                  ...
                  :style="{ color: utilColor(utilPct(g.used, g.total)) }"
                >
                  {{ utilPct(g.used, g.total) }}% ·
              ...
              <div class="zwh-util-track" style="height: 6px">
                <div
                  :class="`zwh-util-fill is-${utilCls(utilPct(g.used, g.total))}`"
                  :style="{ width: utilPct(g.used, g.total) + '%' }"
                />
              </div>
            </div>
          </template>
          <div
            v-else
            style="color: var(--zg-text-dim); font-size: 12px; padding: 8px 0"
          >
    ...
    <div v-if="activity.length">
      <div class="zwh-section-label" style="margin-bottom: 8px">
        Recent Activity
      </div>
      <div class="zen-card">
        <div
        ...
        >
          <span
            class="zwh-sync-dot ok"
            style="margin-top: 5px; flex-shrink: 0"
          />
          <div style="flex: 1; min-width: 0">
            <div style="font-size: 12px; color: var(--zg-text)">
              {{ a.action || a.msg }}
            </div>
            <div
              style="
