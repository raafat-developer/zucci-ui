...
const pct = computed(() =>
  utilPct(props.wh.capacity.used_m2, props.wh.capacity.m2),
);
...
<template>
  <div class="zwh-table-row" @click="emit('click')">
    ...
    >
      ...
      <span class="zwh-table-loc"
        >{{ wh.flag }} {{ wh.city }}, {{ wh.country }}</span
      >
    </div>
    ...
    <div style="display: flex; gap: 3px; flex-wrap: wrap">
      <span
        v-for="t in wh.types"
        :key="t"
        class="zwh-type-badge"
        :style="{
          color: typeMeta(t).color,
          background: typeMeta(t).bg,
          borderColor: typeMeta(t).border,
        }"
      >
        {{ typeMeta(t).label }}
      </span>
    ...
    <!-- Markets -->
    <div style="display: flex; gap: 3px; flex-wrap: wrap">
      <span v-for="m in wh.markets" :key="m" class="zwh-market-tag">{{
        m
      }}</span>
    </div>
    ...
    <div style="display: flex; flex-direction: column; gap: 3px">
      <template v-if="wh.capacity.m2 > 0">
        <div class="zwh-util-track" style="height: 4px">
          <div
            :class="`zwh-util-fill is-${cls}`"
            :style="{ width: pct + '%' }"
          />
        </div>
        <span
          style="font-size: 10px; font-family: var(--zg-mono)"
          :style="{
            color:
              cls === 'high'
                ? 'var(--zg-danger)'
        ...
        >
          {{ pct }}% · {{ wh.capacity.used_m2.toLocaleString() }} /
          {{ wh.capacity.m2.toLocaleString() }} m²
        </span>
      </template>
      <span v-else style="font-size: 11px; color: var(--zg-text-dim)"
        >Virtual</span
      >
    </div>
    ...
    <!-- Status -->
    <span
      class="zwh-status-pill"
      :style="{
        color: statusMeta(wh.status).color,
        background: statusMeta(wh.status).bg,
      }"
    >
