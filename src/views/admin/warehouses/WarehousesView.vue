...
const alerts = computed(() =>
  store.alerts.length ? store.alerts : SEED_ALERTS,
);
...
const countries = computed(() => [
  ...new Set(warehouses.value.map((w) => w.country)),
]);
const allTypes = computed(() => [
  ...new Set(warehouses.value.flatMap((w) => w.types)),
]);

// ── KPI aggregate ─────────────────────────────────────────
const totalWh = computed(() => warehouses.value.length);
const withSpace = computed(() =>
  warehouses.value.filter((w) => w.capacity.m2 > 0),
);
const avgUtil = computed(() => {
  const ws = withSpace.value;
  return ws.length
    ? Math.round(
        ws.reduce((s, w) => s + utilPct(w.capacity.used_m2, w.capacity.m2), 0) /
          ws.length,
      )
    : 0;
});
...
const pendingIn = computed(() =>
  warehouses.value.reduce((s, w) => s + w.stats.pending_inbound, 0),
);
const pendingRet = computed(() =>
  warehouses.value.reduce((s, w) => s + w.stats.pending_returns, 0),
);
const totalSkus = computed(() =>
  warehouses.value.reduce((s, w) => s + w.stats.total_skus, 0),
);
...
const filtered = computed(() =>
  warehouses.value.filter((w) => {
    if (
      ...
      !w.city.toLowerCase().includes(search.value.toLowerCase())
    )
      return false;
    if (typeFilter.value !== "all" && !w.types.includes(typeFilter.value))
      return false;
  ...
  <div v-else class="zwh-wrap">
    ...
    <div class="zwh-header">
      <div class="zwh-header-left">
        ...
        <div class="zwh-subtitle">
          {{ totalWh }} facilities · {{ totalSkus.toLocaleString() }} SKUs ·
    ...
    <div class="zwh-kpi-strip">
      <div class="zwh-kpi-cell">
        ...
        ><span class="zwh-kpi-val">{{ totalWh }}</span>
      </div>
      <div class="zwh-kpi-cell">
        ...
        ><span
          class="zwh-kpi-val"
          :class="
            avgUtil >= 85 ? 'is-danger' : avgUtil >= 60 ? 'is-warn' : 'is-good'
          "
          >{{ avgUtil }}%</span
        >
      </div>
      ...
      <div class="zwh-kpi-cell">
        ...
        ><span class="zwh-kpi-val" :class="{ 'is-warn': pendingIn > 10 }">{{
          pendingIn
        }}</span>
      </div>
      <div class="zwh-kpi-cell">
        <span class="zwh-kpi-lbl">Returns Queue</span
        ><span class="zwh-kpi-val" :class="{ 'is-warn': pendingRet > 5 }">{{
          pendingRet
        }}</span>
      </div>
      <div class="zwh-kpi-cell">
        ...
        ><span
          class="zwh-kpi-val"
          :class="
            dangerAlerts > 0 ? 'is-danger' : totalAlerts > 0 ? 'is-warn' : ''
          "
          >{{ totalAlerts }}</span
        >
      </div>
    ...
    <div class="zwh-toolbar">
      ...
      <button
        ...
        :style="
          typeFilter === t
            ? { borderColor: typeMeta(t).color, color: typeMeta(t).color }
            : {}
        "
      >
        {{ typeMeta(t).label || t }}
      ...
      <div style="margin-left: auto; display: flex; gap: 4px">
        <button
          ...
          title="Grid view"
        >
          <svg viewBox="0 0 12 12" width="12" fill="currentColor">
    ...
    <div class="zwh-content">
      ...
      <div v-else class="zwh-table-wrap">
        <div class="zwh-table-head">
          <span>Warehouse</span><span>Types</span><span>Markets</span
          ><span>Utilization</span><span>SKUs</span><span>Alerts</span
          ><span>Status</span>
        </div>
        <WarehouseTableRow
          v-for="wh in filtered"
    ...
    <ZDrawer
      :open="addOpen"
      title="New Warehouse"
      subtitle="Add a new facility to the network"
      @close="addOpen = false"
    >
      <template #footer>
        <button class="zwh-btn-ghost" @click="addOpen = false">Cancel</button>
        <button class="zwh-btn-primary" @click="createWarehouse">
          Create Warehouse
        </button>
      </template>
      <div
      ...
      >
        <div class="zwh-field">
          <label class="zwh-field-label"
            >Warehouse Name
            <span style="color: var(--zg-danger)">*</span></label
          >
          <input
            class="zwh-field-input"
            v-model="newWh.name"
            placeholder="e.g. Zucci Cairo Hub"
          />
        </div>
        <div class="zwh-field">
          ...
          <select class="zwh-field-input" v-model="newWh.country">
            ...
            <option
              v-for="c in ['AE', 'SA', 'EG', 'KW', 'BH', 'QA', 'OM']"
              :key="c"
              :value="c"
            >
              {{ c }}
        ...
        <div class="zwh-field">
          ...
          <input
            class="zwh-field-input"
            v-model="newWh.operator"
            placeholder="e.g. Aramex Fulfillment"
          />
        </div>
        <div class="zwh-field">
...
.zwh-table-head {
  display: grid;
  grid-template-columns: 1fr 160px 120px 200px 80px 60px 90px;
  padding: 0 16px;
  height: 30px;
  background: var(--zg-panel-hi);
  border-bottom: 1px solid var(--zg-line);
